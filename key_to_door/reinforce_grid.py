...
class ModelHolder:
    ...
    def __init__(self, env, lr):
        ...
        self.discrete = True
        self.observation_dim = env.OBSERVATION_DIM + 1  # current total reward is appended
...
def train(env, lr=0.01):
    ...
    for i_episode in count(1):
        ...
        state = env.reset()
        ep_reward = 0
        state = np.insert(state, 0, ep_reward)  # append current total reward
        t = 0
        done = False

        # run episode
        while done is False and t < max_episode_length:
            t += 1
            action = model_holder.select_action(state)
            state, reward, done = env.step(action)
            model_holder.rewards.append(reward)
            ep_reward += reward
            state = np.insert(state, 0, ep_reward)
...
def main():
    ...
    random_seed = 1
    lr = 5e-4
    env = GridWorld(low_variance=True)
